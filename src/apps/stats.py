...
def plot_altair(max_one):
    violin=alt.Chart(df[df.rating<max_one]).transform_density("price",as_=["price","density"],extent=[0, 1000],groupby=["room_type"]).mark_area(orient='horizontal').encode(
        alt.X('density:Q')
            .stack('center')
            .impute(None)
            .title(None)
...
layout=dbc.Container(
    children=[
        ...
        html.Iframe(
            id='violin',
            srcDoc=plot_altair(max_one=df.rating.max()),
            style={'border-width': '0', 'width': '100%', 'height': '400px'}),
        dcc.Slider(id='xslider', min=df.rating.min(), max=df.rating.max())
            # dbc.Col([
            #         rating
            #     ])
            # ,dbc.Col(number_rate)
...
def update_output(choice1):
    return plot_altair(choice1)